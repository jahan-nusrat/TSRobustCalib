...
def get_montages_from_edf(edf_path: str) -> Tuple[np.ndarray, float, List[str]]:
    ...
    # Decide montage type based on file path
    if "03_tcp_ar_a" in edf_path.lower():
        print(f"Using REDUCED montage for {edf_path} (03_tcp_ar_a lacks A1/A2)")
        ...
        print(f"Using FULL montage for {edf_path}")
        channels = ORDERED_CHANNELS
        montage_indices = MONTAGES_CHANNELS_IDX
        active_montage_names = MONTAGES_NAMES

    # Get ordered channel indices
    try:
    ...
    # Now the montage array has a variable number of channels
